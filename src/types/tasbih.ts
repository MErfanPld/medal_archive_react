/** Tasbih (تسبیح) domain types — /api/tasbih/ */
...
export interface TasbihRequest {
  name: string;
  category_id?: number | null;
  country?: string;
  year?: number | null;
...
export interface TasbihImage {
  id: number;
  image: string;
  image_url: string;
  image_type?: TasbihImageType;
  caption?: string;
  ordering?: number;
  is_primary?: boolean;
...
export interface TasbihPurchaseRecord {
  id: number;
  purchase_date?: string | null;
  location?: string;
  seller?: string;
...
export interface TasbihValuationRecord {
  id: number;
  value: string;
  currency?: Currency;
