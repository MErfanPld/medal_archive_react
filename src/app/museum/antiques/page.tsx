...
export default function MuseumAntiqueArchivePage() {
    ...
    <div>
      <header className="dm-vault">
        <div className="mx-auto max-w-6xl px-5 py-16 sm:px-8 sm:py-24 lg:px-10">
          ...
          <h1 className="museum-serif mt-8 text-5xl font-semibold text-white sm:text-6xl lg:text-7xl">
            Antique
            <br />
            Archive
          </h1>
          <p className="mt-6 max-w-md text-sm leading-7 text-white/55">
            تالار آنتیک — اشیای تاریخی و میراث مادی.
          </p>
        ...
        <div className="mx-auto flex max-w-6xl flex-col gap-3 px-5 py-6 sm:flex-row sm:items-center sm:px-8 lg:px-10">
          ...
          >
            <input
              ...
              placeholder="جستجو…"
              className="h-11 w-full border-0 border-b border-white/15 bg-transparent text-sm text-white outline-none placeholder:text-white/30 focus:border-[#c4a574]"
            />
            <button type="submit" className="h-11 text-sm text-[#c4a574]">جستجو</button>
          </form>
          <select
            className="h-11 border-0 border-b border-white/15 bg-transparent text-sm text-white"
            value={category ?? ""}
            onChange={(e) => {
          ...
          >
            ...
            {categoriesData?.results?.map((c) => (
              <option key={c.id} value={c.id} className="text-text">{c.name}</option>
            ))}
          </select>
          <select
            className="h-11 border-0 border-b border-white/15 bg-transparent text-sm text-white"
        ...
        <div className="mx-auto max-w-6xl px-5 py-14 sm:px-8 lg:px-10">
          {isError ? (
            <div className="py-20 text-center text-white/70">
              ...
              <button type="button" onClick={() => refetch()} className="mt-4 text-[#c4a574]">
                تلاش مجدد
              </button>
            </div>
          ) : isLoading ? (
            <div className="grid grid-cols-2 gap-8 lg:grid-cols-3">
              {Array.from({ length: 6 }).map((_, i) => (
                <div key={i} className="museum-shimmer mx-auto aspect-square max-w-[14rem] rounded-full" />
              ))}
            </div>
          ) : items.length === 0 ? (
            ...
            <>
              <div className="grid grid-cols-2 gap-x-6 gap-y-12 sm:gap-x-10 lg:grid-cols-3">
                {items.map((c) => {
                  ...
                  return (
                    <Link key={c.id} href={`/museum/antiques/${c.id}`} className="group text-center">
                      <div className="museum-object-frame rounded-sm relative mx-auto aspect-square max-w-[13rem] overflow-hidden bg-[#1a1612] lg:max-w-[15rem]">
                        {src ? (
                          ...
                          <img src={src} alt={c.name} className="h-full w-full object-cover transition duration-700 group-hover:scale-105" />
                        ) : (
                          <ItemPlaceholder kind="medal" />
                        )}
                      </div>
                      <p className="dm-label mt-5 text-white/30">{c.catalog_number ? `AN / ${c.catalog_number}` : (c.material || c.country || "—")}</p>
