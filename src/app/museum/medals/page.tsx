...
function MuseumMedalsArchivePage() {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery({
    queryKey: ["museum-medals", page, search],
    enabled: isHydrated,
    queryFn: () =>
      getMedals({
        page,
  ...
  const heroSrc =
    medals.map(imgOf).find(Boolean) || "/brand/study-banner.jpg";
  ...
  return (
    <div className="mu-stage min-h-screen">
      <header className="relative overflow-hidden border-b border-white/5">
        ...
        <div className="mu-container relative z-10 pb-12 pt-10 sm:pb-16 sm:pt-14">
          <nav className="mu-anim-rise text-sm text-white/45">
            ...
            <span className="text-[#C8A75D]">مدال‌ها</span>
          </nav>
          <h1 className="museum-serif mu-anim-rise mt-8 text-4xl font-semibold text-white sm:text-5xl lg:text-6xl">
            آرشیو مدال‌ها
          </h1>
          <p className="mu-anim-rise mt-4 max-w-lg text-sm leading-8 text-white/55">
            مجموعه مدال‌ها و نشان‌های تاریخی — با جستجو در نام، کشور یا سال
            کاوش کنید.
          </p>
      ...
      <div className="mu-container">
        <form
          className="mu-filter-search mu-anim-rise my-8"
          onSubmit={(e) => {
            e.preventDefault();
        ...
        >
          ...
          <input
            ...
            placeholder="جستجو نام، کشور، سال یا شماره کاتالوگ…"
          />
          ...
          {isError ? (
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
              {Array.from({ length: 8 }).map((_, i) => (
                <div key={i} className="museum-shimmer aspect-[4/5] rounded-sm" />
              ))}
            </div>
          ) : medals.length === 0 ? (
            ...
            <>
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 lg:gap-5">
                {medals.map((m) => {
                  const src = imgOf(m);
                  return (
                    <Link key={m.id} href={`/museum/medals/${m.id}`} className="mu-item-card group">
                      <div className="relative aspect-[4/5] overflow-hidden bg-[#0d0d0d]">
                        {src ? (
                          // eslint-disable-next-line @next/next/no-img-element
                          <img src={src} alt={m.name} className="h-full w-full object-cover transition duration-700 group-hover:scale-105" />
                        ) : (
                          <div className="flex h-full w-full flex-col items-center justify-center gap-2 text-[#C8A75D]/50">
                            ...
                            <span className="text-[0.65rem]">بدون تصویر</span>
                          </div>
                        )}
                        <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/10 to-transparent opacity-90" />
                        <div className="absolute inset-x-0 bottom-0 p-4">
                          <p className="text-[0.65rem] text-[#C8A75D]">
                            {m.catalog_number ? `شماره ${m.catalog_number}` : "مدال"}
                          </p>
