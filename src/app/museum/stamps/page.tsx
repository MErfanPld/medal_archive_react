...
export default function MuseumStampArchivePage() {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery({
    ...
    queryFn: () => getStamps({ page, search: search || undefined, category, is_active: true, ordering }), retry: 1,
  });
  ...
  return (
    <div>
      <header className="dm-vault"><div className="mx-auto max-w-6xl px-5 py-16 sm:px-8 sm:py-24 lg:px-10">
        <nav className="text-[0.65rem] tracking-[0.2em] text-white/35"><Link href="/museum" className="hover:text-white">خانه</Link><span className="mx-2">/</span><span>STAMP ARCHIVE</span></nav>
        <h1 className="museum-serif mt-8 text-5xl font-semibold text-white sm:text-6xl lg:text-7xl">Stamp<br />Archive</h1>
        <p className="mt-6 max-w-md text-sm leading-7 text-white/55">تالار تمبرها — چاپ، سری و تاریخ پستی.</p>
      ...
      <div className="dm-vault"><div className="mx-auto max-w-6xl px-5 py-14 sm:px-8 lg:px-10">
        ...
        : (<><div className="grid grid-cols-2 gap-x-6 gap-y-12 sm:gap-x-10 lg:grid-cols-3">{items.map((c) => { const src = resolveMediaUrl(imgOf(c)); return (
          <Link key={c.id} href={`/museum/stamps/${c.id}`} className="group text-center">
            ...
            <p className="dm-label mt-5 text-white/30">{c.catalog_number ? `ST / ${c.catalog_number}` : (c.material || c.country || "—")}</p>
