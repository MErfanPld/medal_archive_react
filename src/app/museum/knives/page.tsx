...
export default function MuseumKnifeArchivePage() {
      ...
      <header className="dm-vault">
        <div className="mx-auto max-w-6xl px-5 py-16 sm:px-8 sm:py-24 lg:px-10">
          ...
          <p className="mt-6 max-w-md text-sm leading-7 text-white/55">
            تالار چاقوها — تیغه، دسته و هنر ساخت.
          </p>
        ...
        <div className="mx-auto max-w-6xl px-5 py-14 sm:px-8 lg:px-10">
          {isError ? (
            <div className="py-20 text-center text-white/70">
              ...
              <button type="button" onClick={() => refetch()} className="mt-4 text-[#c4a574]">تلاش مجدد</button>
            </div>
          ) : isLoading ? (
            ...
            <>
              <div className="grid grid-cols-2 gap-x-6 gap-y-12 sm:gap-x-10 lg:grid-cols-3">
                {items.map((c) => {
                  ...
                  return (
                    <Link key={c.id} href={`/museum/knives/${c.id}`} className="group text-center">
                      ...
                      <p className="dm-label mt-5 text-white/30">{c.catalog_number ? `KN / ${c.catalog_number}` : (c.material || c.country || "—")}</p>
