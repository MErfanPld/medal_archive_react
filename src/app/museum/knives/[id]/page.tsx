...
import { getKnifeById, getKnives, getKnifeImages } from "@/lib/data/knives";
import { resolveMediaUrl } from "@/lib/utils";
import { MuseumDetailLayout } from "@/components/museum/detail-layout";

export default function MuseumKnifeDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  const { data: item, isLoading, isError } = useQuery({
    queryKey: ["mu-knives", itemId],
    enabled: isHydrated && !Number.isNaN(itemId),
    queryFn: () => getKnifeById(itemId),
  });
  const { data: imagesRes } = useQuery({
    queryKey: ["mu-knives-imgs", itemId],
    enabled: isHydrated && !Number.isNaN(itemId) && Boolean(item),
    queryFn: () => getKnifeImages(itemId),
  });
  const { data: related } = useQuery({
    queryKey: ["mu-related-knives"],
  ...
  if (isError || !item) return (<div className="mu-stage px-5 py-24 text-center"><p className="text-[#C8A75D]">اثر یافت نشد.</p><Link href="/museum/knives" className="mt-4 inline-block text-sm text-[#C8A75D]">بازگشت به آرشیو</Link></div>);

  const anyItem = item as Record<string, unknown>;
  const catalog = (anyItem.catalog_number as string) || "";
  const archiveNo = catalog ? `KN-${catalog}` : `KN-${String(item.id).padStart(4, "0")}`;
  const meta = [
  ...
  const relatedList = ((related?.results ?? []) as Array<{ id: number; name: string; country?: string; year?: number | string; primary_image?: string | null; primary_image_url?: string | null; }>)
    ...
    .map((m) => ({ id: m.id, name: m.name, href: `/museum/knives/${m.id}`, image: m.primary_image_url || m.primary_image, subtitle: [m.country, m.year].filter(Boolean).join(" · ") }));

  return (
    <MuseumDetailLayout
      breadcrumb={[{ href: "/museum", label: "خانه" }, { href: "/museum/knives", label: "چاقو" }]}
      archiveLabel={`Knife · ${archiveNo}`}
      title={item.name}
