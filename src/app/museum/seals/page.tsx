...
export default function MuseumSealArchivePage() {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery({
    queryKey: ["museum-seals", page, search, category, ordering],
    enabled: isHydrated,
    queryFn: () => getSeals({ page, search: search || undefined, category, is_active: true, ordering }),
    retry: 1,
  });
  ...
  return (
    <div>
      <header className="dm-vault"><div className="mx-auto max-w-6xl px-5 py-16 sm:px-8 sm:py-24 lg:px-10">
        <nav className="text-[0.65rem] tracking-[0.2em] text-white/35"><Link href="/museum" className="hover:text-white">خانه</Link><span className="mx-2">/</span><span>SEAL ARCHIVE</span></nav>
        <h1 className="museum-serif mt-8 text-5xl font-semibold text-white sm:text-6xl lg:text-7xl">Seal<br />Archive</h1>
        <p className="mt-6 max-w-md text-sm leading-7 text-white/55">تالار مهرها — نقش، کتیبه و اعتبار.</p>
      ...
      <div className="dm-vault"><div className="mx-auto max-w-6xl px-5 py-14 sm:px-8 lg:px-10">
        ...
        : (<><div className="grid grid-cols-2 gap-x-6 gap-y-12 sm:gap-x-10 lg:grid-cols-3">{items.map((c) => { const src = resolveMediaUrl(imgOf(c)); return (
          <Link key={c.id} href={`/museum/seals/${c.id}`} className="group text-center">
            ...
            <p className="dm-label mt-5 text-white/30">{c.catalog_number ? `SL / ${c.catalog_number}` : (c.material || c.country || "—")}</p>
