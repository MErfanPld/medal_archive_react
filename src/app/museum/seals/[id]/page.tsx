...
import { getSealById, getSeals, getSealImages } from "@/lib/data/seals";
import { resolveMediaUrl } from "@/lib/utils";
import { MuseumDetailLayout } from "@/components/museum/detail-layout";

export default function MuseumSealDetailPage({
  params,
}: {
  ...
  const { data: item, isLoading, isError } = useQuery({
    queryKey: ["mu-seals", itemId],
    enabled: isHydrated && !Number.isNaN(itemId),
    queryFn: () => getSealById(itemId),
  });

  const { data: imagesRes } = useQuery({
    queryKey: ["mu-seals-imgs", itemId],
    enabled: isHydrated && !Number.isNaN(itemId) && Boolean(item),
    queryFn: () => getSealImages(itemId),
  });

  const { data: related } = useQuery({
    queryKey: ["mu-related-seals"],
  ...
  const images = useMemo(() => {
    const raw = imagesRes as
      | { results?: Array<{ id: number; image?: string; image_url?: string }> }
      | Array<{ id: number; image?: string; image_url?: string }>
      | undefined;
  ...
  if (isError || !item) {
    return (
      <div className="mu-stage px-5 py-24 text-center">
        ...
        <Link href="/museum/seals" className="mt-4 inline-block text-sm text-[#C8A75D]">بازگشت به آرشیو</Link>
      </div>
    );
  }
  ...
  const archiveNo = catalog ? `SL-${catalog}` : `SL-${String(item.id).padStart(4, "0")}`;
  const meta = [
  ...
  const relatedList = ((related?.results ?? []) as Array<{ id: number; name: string; country?: string; year?: number | string; primary_image?: string | null; primary_image_url?: string | null; }>)
    ...
    .slice(0, 4)
    .map((m) => ({ id: m.id, name: m.name, href: `/museum/seals/${m.id}`, image: m.primary_image_url || m.primary_image, subtitle: [m.country, m.year].filter(Boolean).join(" · ") }));

  return (
    <MuseumDetailLayout
      breadcrumb={[{ href: "/museum", label: "خانه" }, { href: "/museum/seals", label: "مهر" }]}
      archiveLabel={`Seal · ${archiveNo}`}
      title={item.name}
