...
function MuseumTasbihArchivePage() {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery({
    ...
    queryFn: () => getTasbihs({ page, search: search || undefined, category, ordering }), retry: 1,
  });
      ...
      <header className="mu-archive-hero">
        <div className="mu-container">
          <nav className="mu-anim-rise text-[0.65rem] tracking-[0.2em] text-white/35">
            ...
            <span className="text-[#C8A75D]">TASBIH ARCHIVE</span>
          </nav>
          <h1 className="museum-serif mu-anim-rise mt-8 text-5xl font-semibold text-white sm:text-6xl lg:text-7xl">Tasbih<br />Archive</h1>
          <p className="mu-anim-rise mt-6 max-w-md text-sm leading-7 text-white/55">تالار تسبیح‌ها — دانه، جنس و سنت.</p>
        ...
        <div className="py-12 sm:py-16">
          ...
          : (<><div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 lg:gap-5">
              {items.map((m) => { const src = imgOf(m); return (
                <Link key={m.id} href={`/museum/tasbih/${m.id}`} className="mu-item-card">
                  ...
                  <div className="space-y-1 p-4"><p className="text-[0.65rem] tracking-[0.16em] text-[#C8A75D]">{(m as { catalog_number?: string }).catalog_number ? `TB / ${(m as { catalog_number?: string }).catalog_number}` : "تسبیح"}</p>
