...
function MuseumBanknoteArchivePage() {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery({
    queryKey: ["museum-banknotes", page, search, category, ordering],
    enabled: isHydrated,
    queryFn: () => getBanknotes({ page, search: search || undefined, category, ordering }),
    retry: 1,
  });
    ...
    <div className="mu-stage min-h-screen">
      <header className="mu-archive-hero">
        <div className="mu-container">
          <nav className="mu-anim-rise text-[0.65rem] tracking-[0.2em] text-white/35">
            ...
            <span className="text-[#C8A75D]">BANKNOTE ARCHIVE</span>
          </nav>
          <h1 className="museum-serif mu-anim-rise mt-8 text-5xl font-semibold text-white sm:text-6xl lg:text-7xl">Banknote<br />Archive</h1>
          <p className="mu-anim-rise mt-6 max-w-md text-sm leading-7 text-white/55">تالار اسکناس‌ها — کاغذ، نقش و تاریخ اقتصاد.</p>
      ...
      <div className="mu-container">
        ...
        <div className="py-12 sm:py-16">
          {isError ? (
            <div className="py-20 text-center text-white/70"><p>{(error as Error)?.message}</p><button type="button" onClick={() => refetch()} className="mt-4 text-[#C8A75D]">تلاش مجدد</button></div>
          ) : isLoading ? (
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">{Array.from({ length: 8 }).map((_, i) => (<div key={i} className="museum-shimmer aspect-square rounded-sm" />))}</div>
          ) : items.length === 0 ? (
            ...
            <>
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 lg:gap-5">
                {items.map((m) => {
                  const src = imgOf(m);
                  return (
                    <Link key={m.id} href={`/museum/banknotes/${m.id}`} className="mu-item-card">
                      <div className="relative aspect-square overflow-hidden bg-[#0d0d0d]">
                        {src ? (/* eslint-disable-next-line @next/next/no-img-element */<img src={src} alt={m.name} className="h-full w-full object-cover" />) : (<div className="flex h-full w-full flex-col items-center justify-center gap-2 text-[#C8A75D]/50"><ImageOff className="size-8" strokeWidth={1.25} /><span className="text-[0.65rem] tracking-[0.16em] uppercase">بدون تصویر</span></div>)}
                      </div>
                      <div className="space-y-1 p-4">
                        <p className="text-[0.65rem] tracking-[0.16em] text-[#C8A75D]">{(m as { catalog_number?: string }).catalog_number ? `BN / ${(m as { catalog_number?: string }).catalog_number}` : "اسکناس"}</p>
                        <h3 className="line-clamp-2 text-sm font-medium text-[#F5F2EA]">{m.name}</h3>
                        <p className="text-xs text-[#A8A8A8]">{[m.country, m.year].filter(Boolean).join(" · ") || "—"}</p>
                      </div>
                    </Link>
                  );
                })}
...
export default function Page() {
  return (<Suspense fallback={<div className="min-h-screen bg-[#0D0D0D] museum-shimmer" />}><MuseumBanknoteArchivePage /></Suspense>);
}
