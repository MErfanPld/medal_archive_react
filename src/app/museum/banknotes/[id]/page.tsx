...
import { getBanknoteById, getBanknotes, getBanknoteImages } from "@/lib/data/banknotes";
import { resolveMediaUrl } from "@/lib/utils";
import { MuseumDetailLayout } from "@/components/museum/detail-layout";

export default function MuseumBanknoteDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  const { data: item, isLoading, isError } = useQuery({
    queryKey: ["mu-banknotes", itemId],
    enabled: isHydrated && !Number.isNaN(itemId),
    queryFn: () => getBanknoteById(itemId),
  });
  const { data: imagesRes } = useQuery({
    queryKey: ["mu-banknotes-imgs", itemId],
    enabled: isHydrated && !Number.isNaN(itemId) && Boolean(item),
    queryFn: () => getBanknoteImages(itemId),
  });
  const { data: related } = useQuery({
    queryKey: ["mu-related-banknotes"],
  ...
  if (isError || !item) return (<div className="mu-stage px-5 py-24 text-center"><p className="text-[#C8A75D]">اثر یافت نشد.</p><Link href="/museum/banknotes" className="mt-4 inline-block text-sm text-[#C8A75D]">بازگشت به آرشیو</Link></div>);

  const anyItem = item as Record<string, unknown>;
  const catalog = (anyItem.catalog_number as string) || "";
  const archiveNo = catalog ? `BN-${catalog}` : `BN-${String(item.id).padStart(4, "0")}`;
  const meta = [
  ...
  const relatedList = ((related?.results ?? []) as Array<{ id: number; name: string; country?: string; year?: number | string; primary_image?: string | null; primary_image_url?: string | null; }>)
    ...
    .map((m) => ({ id: m.id, name: m.name, href: `/museum/banknotes/${m.id}`, image: m.primary_image_url || m.primary_image, subtitle: [m.country, m.year].filter(Boolean).join(" · ") }));

  return (
    <MuseumDetailLayout
      breadcrumb={[{ href: "/museum", label: "خانه" }, { href: "/museum/banknotes", label: "اسکناس" }]}
      archiveLabel={`Banknote · ${archiveNo}`}
      title={item.name}
