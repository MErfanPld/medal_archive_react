...
import { getRingById, getRings, getRingImages } from "@/lib/data/rings";
import { resolveMediaUrl } from "@/lib/utils";
import { MuseumDetailLayout } from "@/components/museum/detail-layout";

export default function MuseumRingDetailPage({
  params,
}: {
  ...
  const { data: item, isLoading, isError } = useQuery({
    queryKey: ["mu-rings", itemId],
    enabled: isHydrated && !Number.isNaN(itemId),
    queryFn: () => getRingById(itemId),
  });

  const { data: imagesRes } = useQuery({
    queryKey: ["mu-rings-imgs", itemId],
    enabled: isHydrated && !Number.isNaN(itemId) && Boolean(item),
    queryFn: () => getRingImages(itemId),
  });

  const { data: related } = useQuery({
    queryKey: ["mu-related-rings"],
  ...
  if (isError || !item) {
    return (
      <div className="mu-stage px-5 py-24 text-center">
        ...
        <Link href="/museum/rings" className="mt-4 inline-block text-sm text-[#C8A75D]">
          بازگشت به آرشیو
        </Link>
      </div>
    );
  ...
  const archiveNo = catalog
    ? `RG-${catalog}`
    : `RG-${String(item.id).padStart(4, "0")}`;
  ...
  const relatedList = (
    ...
    .map((m) => ({
      id: m.id,
      name: m.name,
      href: `/museum/rings/${m.id}`,
      image: m.primary_image_url || m.primary_image,
