...
function MuseumRingArchivePage() {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery({
    queryKey: ["museum-rings", page, search],
    enabled: isHydrated,
    queryFn: () => getRings({ page, search: search || undefined, ordering: "-created_at" }),
    retry: 1,
  });
      ...
      <header className="relative overflow-hidden border-b border-white/5">
        ...
        <div className="mu-container relative z-10 pb-12 pt-10 sm:pb-16 sm:pt-14">
          <nav className="mu-anim-rise text-sm text-white/45">
            ...
            <span className="text-[#C8A75D]">انگشترها</span>
          </nav>
          <h1 className="museum-serif mu-anim-rise mt-8 text-4xl font-semibold text-white sm:text-5xl lg:text-6xl">آرشیو انگشترها</h1>
          <p className="mu-anim-rise mt-4 max-w-lg text-sm leading-8 text-white/55">مجموعه انگشترهای تاریخی — با جستجو در نام، کشور یا سال کاوش کنید.</p>
        ...
        <div className="pb-16">
          ...
          : (<><div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 lg:gap-5">
              {items.map((m) => { const src = imgOf(m); return (
                <Link key={m.id} href={`/museum/rings/${m.id}`} className="mu-item-card group">
                  <div className="relative aspect-[4/5] overflow-hidden bg-[#0d0d0d]">
                    ...
                    <div className="absolute inset-x-0 bottom-0 p-4">
                      <p className="text-[0.65rem] text-[#C8A75D]">{(m as { catalog_number?: string }).catalog_number ? `شماره ${(m as { catalog_number?: string }).catalog_number}` : "انگشتر"}</p>
