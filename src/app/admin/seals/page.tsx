...
import { Plus, Eye, Pencil, Trash2, Hexagon, ImageOff } from "lucide-react";
import {
  ListFilters,
  FilterSearchField,
  FilterSelect,
...
import { getSeals, deleteSeal } from "@/lib/data/seals";
import { getCategories } from "@/lib/data/categories";
import { formatNumber, resolvePrimaryImage } from "@/lib/utils";
...
} from "@/lib/seal-labels";
import type { Seal } from "@/types/seals";

function SealThumb({ item }: { item: Seal }) {
...
export default function SealsPage() {
  ...
  const canCreate = hasPermission(PERMISSIONS.SEALS_CREATE);
  const canUpdate = hasPermission(PERMISSIONS.SEALS_UPDATE);
  const canDelete = hasPermission(PERMISSIONS.SEALS_DELETE);
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ["seals", page, search, authenticity, category],
    queryFn: () =>
      getSeals({
        page,
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => deleteSeal(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["seals"] });
      toast.success("مهر با موفقیت حذف شد");
      setDeleteId(null);
    },
    onError: (err) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-page-title">مهر</h1>
          <p className="mt-1 text-caption">مدیریت مجموعه مهرها</p>
        </div>
        {canCreate && (
          <Button asChild>
            <Link href="/admin/seals/new">
      ...
      {isError && (
        <Alert variant="danger">
          <span>خطا در بارگذاری مهرها</span>
      ...
      {isLoading ? (
        ...
        <EmptyState
          icon={Hexagon}
          title="مهری یافت نشد"
          description="هنوز مهری در آرشیو نیست یا با فیلتر فعلی مطابقت ندارد."
          action={
            canCreate ? (
              <Button asChild>
                <Link href="/admin/seals/new">افزودن مهر</Link>
              </Button>
            ) : undefined
          }
        />
        ...
        <div className="overflow-x-auto rounded-xl border border-border">
          <table className="w-full text-sm">
            <thead className="bg-surface-muted/60 text-text-muted">
              <tr>
                <th className="p-3 text-right font-medium">مهر</th>
                ...
                <CardContent className="space-y-2 p-4">
                  ...
                  <div className="flex gap-1 pt-1">
                    <Button variant="outline" size="sm" asChild>
                      <Link href={`/admin/seals/${item.id}`}>مشاهده</Link>
                    </Button>
                    {canUpdate && (
                      <Button variant="ghost" size="sm" asChild>
                        <Link href={`/admin/seals/${item.id}/edit`}>ویرایش</Link>
                      </Button>
                    )}
                  </div>
                </CardContent>
      ...
      <ConfirmDialog
        ...
        title="حذف مهر"
        description="آیا از حذف این مهر مطمئن هستید؟ این عمل قابل بازگشت نیست."
        confirmLabel="حذف"
        variant="danger"
