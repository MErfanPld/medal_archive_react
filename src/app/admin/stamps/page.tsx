...
import { getStamps, deleteStamp } from "@/lib/data/stamps";
...
function StampThumb({ item }: { item: Stamp }) {
...
export default function StampsPage() {
  ...
  const canCreate = hasPermission(PERMISSIONS.STAMPS_CREATE);
  const canUpdate = hasPermission(PERMISSIONS.STAMPS_UPDATE);
  const canDelete = hasPermission(PERMISSIONS.STAMPS_DELETE);
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ["stamps", page, search, category, authenticity],
    queryFn: () =>
      getStamps({
        page,
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => deleteStamp(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["stamps"] });
      setDeleteId(null);
      toast.success("تمبر با موفقیت حذف شد");
    },
    onError: (err) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-page-title">تمبر</h1>
          <p className="mt-1 text-caption">مدیریت مجموعه تمبرها</p>
        </div>
        {canCreate && (
          <Button asChild>
      ...
      {isLoading ? (
        ...
        <Alert variant="danger" className="flex items-center justify-between gap-3">
          <span>خطا در بارگذاری تمبرها</span>
        ...
        <EmptyState
          title="تمبری یافت نشد"
          description="هنوز تمبری در آرشیو نیست یا با فیلتر فعلی مطابقت ندارد."
          icon={<Stamp className="size-10" />}
          action={
            canCreate ? (
              <Button asChild>
                <Link href="/admin/stamps/new">افزودن تمبر</Link>
              </Button>
            ) : undefined
          }
        />
            ...
            <div className="hidden overflow-x-auto rounded-xl border border-border bg-surface md:block">
              <table className="w-full text-sm">
                <thead className="bg-surface-muted/50">
                  <tr>
                    <th className="p-3 text-right font-medium">تمبر</th>
                    ...
                    >
                      <td className="p-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <Link
                              href={`/admin/stamps/${c.id}`}
                              className="font-medium hover:text-primary"
                            >
                              {c.name}
                            </Link>
                            <p className="text-xs text-text-muted">
                              {c.catalog_number || c.series || c.material || "—"}
                            </p>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="p-3">
                        <div className="flex items-center gap-1">
                          <Button variant="ghost" size="sm" asChild>
                            <Link href={`/admin/stamps/${c.id}`} aria-label="مشاهده">
                ...
                <Card key={c.id} className="overflow-hidden">
                  <div className="flex h-36 items-center justify-center bg-surface-muted">
                    {resolvePrimaryImage(c) ? (
                      ...
                      <Stamp className="size-10 text-text-subtle" />
                    )}
                  </div>
      ...
      <ConfirmDialog
        ...
        title="حذف تمبر"
        description="آیا از حذف این تمبر مطمئن هستید؟ این عمل قابل بازگشت نیست."
        confirmLabel="حذف"
        onConfirm={async () => {
