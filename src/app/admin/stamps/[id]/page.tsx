...
export default function StampDetailPage({
  ...
  const imagesQ = useQuery({
    queryKey: ["stamp-images", stampId],
    queryFn: () => getStampImages(stampId),
    enabled: !Number.isNaN(stampId),
  });

  const uploadMutation = useMutation({
    mutationFn: (fd: FormData) => uploadStampImage(stampId, fd),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["stamp-images", stampId] });
  ...
  const deleteImageMutation = useMutation({
    mutationFn: (imageId: number) => deleteStampImage(stampId, imageId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["stamp-images", stampId] });
      queryClient.invalidateQueries({ queryKey: ["stamp", stampId] });
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <Button variant="ghost" size="icon" asChild>
            <Link href="/admin/stamps">
              <ArrowRight className="size-5" />
            </Link>
          </Button>
          <div>
            <h1 className="text-xl font-semibold text-text">{item.name}</h1>
            <p className="text-sm text-text-muted">تمبر · #{item.id}</p>
          </div>
        </div>
        {canUpdate && (
          <Button asChild>
            <Link href={`/admin/stamps/${item.id}/edit`}>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Card className="overflow-hidden lg:col-span-1">
          <div className="flex aspect-square items-center justify-center bg-surface-muted">
            {primary ? (
              ...
              <Stamp className="size-16 text-text-subtle" />
            )}
          </div>
        </Card>
        ...
        <Card className="lg:col-span-2">
          ...
          <CardContent>
            ...
            <Field label="ارزش اسمی" value={item.denomination} />
            <Field label="سری / مجموعه" value={item.series} />
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between gap-1 p-2 text-xs">
                      <span className="truncate text-text-muted">
                        {stampImageTypeOptions.find((o) => o.value === img.image_type)?.label ||
                          img.image_type ||
                          "—"}
                      </span>
                      {canUpdate && (
