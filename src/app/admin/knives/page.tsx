...
import { getKnives, deleteKnife } from "@/lib/data/knives";
...
import type { Knife } from "@/types/knives";

function KnifeThumb({ item }: { item: Knife }) {
...
export default function KnivesPage() {
  ...
  const canCreate = hasPermission(PERMISSIONS.KNIVES_CREATE);
  const canUpdate = hasPermission(PERMISSIONS.KNIVES_UPDATE);
  const canDelete = hasPermission(PERMISSIONS.KNIVES_DELETE);
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ["knives", page, search, category, authenticity],
    queryFn: () =>
      getKnives({
        page,
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => deleteKnife(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["knives"] });
      setDeleteId(null);
      toast.success("چاقو با موفقیت حذف شد");
    },
    onError: (err) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-page-title">چاقو</h1>
          <p className="mt-1 text-caption">مدیریت مجموعه چاقوها</p>
        </div>
        {canCreate && (
          <Button asChild>
            <Link href="/admin/knives/new">
      ...
      {isLoading ? (
        ...
        <Alert variant="danger" className="flex items-center justify-between gap-3">
          <span>خطا در بارگذاری چاقوها</span>
        ...
        <EmptyState
          title="چاقویی یافت نشد"
          description="هنوز چاقویی در آرشیو نیست یا با فیلتر فعلی مطابقت ندارد."
          icon={<Sword className="size-10" />}
          action={
            canCreate ? (
              <Button asChild>
                <Link href="/admin/knives/new">افزودن چاقو</Link>
              </Button>
            ) : undefined
          }
        />
            ...
            <div className="hidden overflow-x-auto rounded-xl border border-border bg-surface md:block">
              <table className="w-full text-sm">
                <thead className="bg-surface-muted/50">
                  <tr>
                    <th className="p-3 text-right font-medium">چاقو</th>
                      ...
                      <td className="p-3">
                        <div className="flex items-center gap-1">
                          <Button variant="ghost" size="sm" asChild>
                            <Link href={`/admin/knives/${c.id}`} aria-label="مشاهده">
              ...
              <Card key={c.id} className="overflow-hidden">
                <div className="flex h-36 items-center justify-center bg-surface-muted">
                  {resolvePrimaryImage(c) ? (
                    ...
                    <Sword className="size-10 text-text-subtle" />
                  )}
                </div>
      ...
      <ConfirmDialog
        ...
        title="حذف چاقو"
        description="آیا از حذف این چاقو مطمئن هستید؟ این عمل قابل بازگشت نیست."
        confirmLabel="حذف"
        onConfirm={async () => {
