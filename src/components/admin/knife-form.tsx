...
import type { Knife, KnifeRequest } from "@/types/knives";
...
const knifeSchema = z.object({
  ...
  blade_length: z.string().optional().nullable(),
  handle_material: z.string().optional(),
...
export type KnifeFormValues = z.infer<typeof knifeSchema>;

function toFormDefaults(item?: Knife | null): KnifeFormValues {
  if (!item) {
    return {
      name: "",
      category_id: null,
...
function toRequest(values: KnifeFormValues): KnifeRequest {
  return {
    ...
    quality: (values.quality || undefined) as KnifeRequest["quality"],
    preservation_condition: values.preservation_condition || undefined,
    authenticity: (values.authenticity || undefined) as KnifeRequest["authenticity"],
    ...
    purchase_currency:
      (values.purchase_currency as KnifeRequest["purchase_currency"]) || undefined,
...
interface KnifeFormProps {
  knife?: Knife | null;
  onSubmit: (data: KnifeRequest) => Promise<void>;
  onCancel: () => void;
  loading?: boolean;
  submitLabel?: string;
}
...
export function KnifeForm({
  ...
  const {
    ...
    control,
    formState: { errors },
  } = useForm<KnifeFormValues>({
    resolver: zodResolver(knifeSchema),
    defaultValues: toFormDefaults(knife),
  });
