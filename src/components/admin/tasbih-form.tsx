...
import type { Tasbih, TasbihRequest } from "@/types/tasbih";
...
const schema = z.object({
  ...
  bead_count: z.string().optional().nullable(),
  bead_material: z.string().optional(),
...
function toDefaults(item?: Tasbih | null): FormValues {
  return {
    name: item?.name ?? "",
...
function toRequest(v: FormValues): TasbihRequest {
  return {
    name: v.name.trim(),
    ...
    bead_count: v.bead_count || null,
    bead_material: v.bead_material || undefined,
    dimensions: v.dimensions || undefined,
    weight: v.weight || null,
    maker: v.maker || undefined,
    catalog_number: v.catalog_number || undefined,
    quality: (v.quality || undefined) as TasbihRequest["quality"],
    authenticity: (v.authenticity || undefined) as TasbihRequest["authenticity"],
    notes: v.notes || undefined,
    is_active: v.is_active ?? true,
    purchase_currency: (v.purchase_currency as TasbihRequest["purchase_currency"]) || undefined,
...
interface Props {
  tasbih?: Tasbih | null;
  onSubmit: (data: TasbihRequest) => Promise<void>;
  onCancel: () => void;
  loading?: boolean;
  submitLabel?: string;
}

export function TasbihForm({ tasbih, onSubmit, onCancel, loading, submitLabel = "ذخیره" }: Props) {
  const { register, handleSubmit, control, formState: { errors } } = useForm<FormValues>({
    resolver: zodResolver(schema),
    defaultValues: toDefaults(tasbih),
  });
      ...
      <Card>
        ...
        <CardContent className="grid gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            <Input className="mt-1.5" {...register("bead_count")} placeholder="مثلاً ۳۳" />
          </div>
          <div>
