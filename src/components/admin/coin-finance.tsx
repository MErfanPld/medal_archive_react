...
interface CoinFinanceProps {
  coinId: number;
  canEdit?: boolean;
}

export function CoinFinance({
  coinId,
  canEdit = false,
}: CoinFinanceProps) {
  ...
  const { data: purchases = [], isLoading: pLoading } = useQuery({
    queryKey: ["coin-purchases", coinId],
    queryFn: () => getCoinPurchases(coinId),
  });

  const { data: valuations = [], isLoading: vLoading } = useQuery({
    queryKey: ["coin-valuations", coinId],
    queryFn: () => getCoinValuations(coinId),
  });

  const createPurchaseMutation = useMutation({
    mutationFn: () =>
      createCoinPurchase(coinId, {
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["coin-purchases", coinId] });
      toast.success("سابق خرید ثبت شد");
    ...
    onError: (err) => {
      toast.error(getErrorMessage(err, "خطا در ثبت سابقه خرید"));
    },
  });

  const deletePurchaseMutation = useMutation({
    mutationFn: (id: number) => deleteCoinPurchase(coinId, id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["coin-purchases", coinId] });
    ...
    onError: (err) => {
      toast.error(getErrorMessage(err, "خطا در حذف"));
    },
  });

  const createValuationMutation = useMutation({
    mutationFn: () =>
      createCoinValuation(coinId, {
    ...
    onError: (err) => {
      toast.error(getErrorMessage(err, "خطا در ثبت ارزش‌گذاری"));
    },
  });

  const deleteValuationMutation = useMutation({
    mutationFn: (id: number) => deleteCoinValuation(coinId, id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["coin-valuations", coinId] });
