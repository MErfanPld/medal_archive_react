...
import {
  ...
  createStampPurchase,
  deleteStampPurchase,
  createStampValuation,
  deleteStampValuation,
} from "@/lib/data/stamps";
...
interface StampFinanceProps {
  stampId: number;
  canEdit?: boolean;
}

export function StampFinance({
  stampId,
  canEdit = false,
}: StampFinanceProps) {
  ...
  const { data: purchases = [], isLoading: pLoading } = useQuery({
    queryKey: ["stamp-purchases", stampId],
    queryFn: () => getStampPurchases(stampId),
  });

  const { data: valuations = [], isLoading: vLoading } = useQuery({
    queryKey: ["stamp-valuations", stampId],
    queryFn: () => getStampValuations(stampId),
  });

  const createPurchaseMutation = useMutation({
    mutationFn: () =>
      createStampPurchase(stampId, {
  ...
  const deletePurchaseMutation = useMutation({
    mutationFn: (id: number) => deleteStampPurchase(stampId, id),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["stamp-purchases", stampId],
      });
  ...
  const deleteValuationMutation = useMutation({
    mutationFn: (id: number) => deleteStampValuation(stampId, id),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["stamp-valuations", stampId],
      });
