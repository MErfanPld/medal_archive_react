...
import {
  getAntiquePurchases,
  getAntiqueValuations,
  createAntiquePurchase,
  deleteAntiquePurchase,
  createAntiqueValuation,
  deleteAntiqueValuation,
...
interface AntiqueFinanceProps {
  antiqueId: number;
  canEdit?: boolean;
}

export function AntiqueFinance({
  antiqueId,
  canEdit = false,
}: AntiqueFinanceProps) {
  ...
  const { data: purchases = [], isLoading: pLoading } = useQuery({
    queryKey: ["antique-purchases", antiqueId],
    queryFn: () => getAntiquePurchases(antiqueId),
  });

  const { data: valuations = [], isLoading: vLoading } = useQuery({
    queryKey: ["antique-valuations", antiqueId],
    queryFn: () => getAntiqueValuations(antiqueId),
  });

  const createPurchaseMutation = useMutation({
    mutationFn: () =>
      createAntiquePurchase(antiqueId, {
    ...
    onError: (err) => {
      toast.error(
        err instanceof ApiError ? err.message : "خطا در ثبت سابقه خرید"
      );
    },
  });

  const deletePurchaseMutation = useMutation({
    mutationFn: (id: number) => deleteAntiquePurchase(antiqueId, id),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["antique-purchases", antiqueId],
      });
  ...
  const createValuationMutation = useMutation({
    mutationFn: () =>
      createAntiqueValuation(antiqueId, {
    ...
    onError: (err) => {
      toast.error(
        err instanceof ApiError ? err.message : "خطا در ثبت ارزش‌گذاری"
      );
    },
  });

  const deleteValuationMutation = useMutation({
    mutationFn: (id: number) => deleteAntiqueValuation(antiqueId, id),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["antique-valuations", antiqueId],
      });
                ...
                >
                  <div>
                    <p className="font-medium">
                      {p.price ? formatNumber(p.price) : "—"} {p.currency}
                    </p>
                    <p className="text-xs text-text-muted">
                      {[formatDate(p.purchase_date), p.seller, p.location]
                        .filter(Boolean)
                        .join(" · ")}
                    </p>
                  </div>
        ...
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          {canEdit && (
            <Button
              size="sm"
              variant="outline"
              onClick={() => setShowValuationForm((v) => !v)}
            >
              <Plus className="size-4" />
              افزودن
            </Button>
        ...
        <CardContent className="space-y-3">
          {showValuationForm && (
            <div className="space-y-2 rounded-lg border border-border bg-surface-muted/40 p-3">
              ...
              <Button
                size="sm"
                loading={createValuationMutation.isPending}
                disabled={!vValue.trim()}
                onClick={() => createValuationMutation.mutate()}
              >
                ثبت
              </Button>
            </div>
              ...
              {valuations.map((v) => (
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-text-muted">
                      {[formatDate(v.valuation_date), v.source]
                        .filter(Boolean)
                        .join(" · ")}
                    </p>
                  </div>
                  {canEdit && (
                    <Button
                      size="sm"
                      variant="ghost"
                      onClick={() => setDeleteValuationId(v.id)}
                    >
                      <Trash2 className="size-4 text-danger" />
                    </Button>
                  )}
