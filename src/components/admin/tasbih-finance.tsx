...
import {
  getTasbihPurchases,
  getTasbihValuations,
  createTasbihPurchase,
  deleteTasbihPurchase,
  createTasbihValuation,
  deleteTasbihValuation,
...
interface TasbihFinanceProps {
  tasbihId: number;
  canEdit?: boolean;
}

export function TasbihFinance({
  tasbihId,
  canEdit = false,
}: TasbihFinanceProps) {
  ...
  const { data: purchases = [], isLoading: pLoading } = useQuery({
    queryKey: ["tasbih-purchases", tasbihId],
    queryFn: () => getTasbihPurchases(tasbihId),
  });

  const { data: valuations = [], isLoading: vLoading } = useQuery({
    queryKey: ["tasbih-valuations", tasbihId],
    queryFn: () => getTasbihValuations(tasbihId),
  });

  const createPurchaseMutation = useMutation({
    mutationFn: () =>
      createTasbihPurchase(tasbihId, {
  ...
  const deletePurchaseMutation = useMutation({
    mutationFn: (id: number) => deleteTasbihPurchase(tasbihId, id),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["tasbih-purchases", tasbihId],
      });
  ...
  const createValuationMutation = useMutation({
    mutationFn: () =>
      createTasbihValuation(tasbihId, {
  ...
  const deleteValuationMutation = useMutation({
    mutationFn: (id: number) => deleteTasbihValuation(tasbihId, id),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["tasbih-valuations", tasbihId],
      });
