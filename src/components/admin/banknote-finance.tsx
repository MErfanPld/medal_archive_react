...
import {
  getBanknotePurchases,
  getBanknoteValuations,
  createBanknotePurchase,
  deleteBanknotePurchase,
  createBanknoteValuation,
  deleteBanknoteValuation,
...
interface BanknoteFinanceProps {
  banknoteId: number;
  canEdit?: boolean;
}

export function BanknoteFinance({
  banknoteId,
  canEdit = false,
}: BanknoteFinanceProps) {
  ...
  const { data: purchases = [], isLoading: pLoading } = useQuery({
    queryKey: ["banknote-purchases", banknoteId],
    queryFn: () => getBanknotePurchases(banknoteId),
  });

  const { data: valuations = [], isLoading: vLoading } = useQuery({
    queryKey: ["banknote-valuations", banknoteId],
    queryFn: () => getBanknoteValuations(banknoteId),
  });

  const createPurchaseMutation = useMutation({
    mutationFn: () =>
      createBanknotePurchase(banknoteId, {
  ...
  const deletePurchaseMutation = useMutation({
    mutationFn: (id: number) => deleteBanknotePurchase(banknoteId, id),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["banknote-purchases", banknoteId],
      });
  ...
  const createValuationMutation = useMutation({
    mutationFn: () =>
      createBanknoteValuation(banknoteId, {
  ...
  const deleteValuationMutation = useMutation({
    mutationFn: (id: number) => deleteBanknoteValuation(banknoteId, id),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["banknote-valuations", banknoteId],
      });
