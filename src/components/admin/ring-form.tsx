...
import type { Ring, RingRequest } from "@/types/rings";
...
const schema = z.object({
  ...
  ring_size: z.string().optional().nullable(),
...
function toDefaults(item?: Ring | null): FormValues {
  return {
    name: item?.name ?? "",
...
function toRequest(v: FormValues): RingRequest {
  return {
    name: v.name.trim(),
    ...
    quality: (v.quality || undefined) as RingRequest["quality"],
    authenticity: (v.authenticity || undefined) as RingRequest["authenticity"],
    notes: v.notes || undefined,
    is_active: v.is_active ?? true,
    purchase_currency: (v.purchase_currency as RingRequest["purchase_currency"]) || undefined,
...
interface Props {
  ring?: Ring | null;
  onSubmit: (data: RingRequest) => Promise<void>;
  onCancel: () => void;
  loading?: boolean;
  submitLabel?: string;
}

export function RingForm({ ring, onSubmit, onCancel, loading, submitLabel = "ذخیره" }: Props) {
  const { register, handleSubmit, control, formState: { errors } } = useForm<FormValues>({
    resolver: zodResolver(schema),
    defaultValues: toDefaults(ring),
  });
    ...
    >
      <Card>
        ...
        <CardContent className="grid gap-4 sm:grid-cols-2">
          ...
          <div>
            <Label>سایز انگشتر</Label>
            <Input className="mt-1.5" {...register("ring_size")} placeholder="مثلاً ۵۴" />
          </div>
          <div>
            <Label>نگین / سنگ</Label>
            <Input className="mt-1.5" {...register("gemstone")} />
          </div>
          <div>
            <Label>ابعاد</Label>
