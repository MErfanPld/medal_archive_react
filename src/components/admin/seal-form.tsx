...
import type { Seal, SealRequest } from "@/types/seals";
...
function toDefaults(item?: Seal | null): FormValues {
  return {
    name: item?.name ?? "",
...
function toRequest(v: FormValues): SealRequest {
  return {
    name: v.name.trim(),
    ...
    quality: (v.quality || undefined) as SealRequest["quality"],
    authenticity: (v.authenticity || undefined) as SealRequest["authenticity"],
    notes: v.notes || undefined,
    is_active: v.is_active ?? true,
    purchase_currency: (v.purchase_currency as SealRequest["purchase_currency"]) || undefined,
...
interface Props {
  seal?: Seal | null;
  onSubmit: (data: SealRequest) => Promise<void>;
  onCancel: () => void;
  loading?: boolean;
  submitLabel?: string;
}

export function SealForm({ seal, onSubmit, onCancel, loading, submitLabel = "ذخیره" }: Props) {
  const { register, handleSubmit, control, formState: { errors } } = useForm<FormValues>({
    resolver: zodResolver(schema),
    defaultValues: toDefaults(seal),
  });
